...
(function () {
  ...
  // Jalankan simpan Erzap di dokumen/window iframe edit. Return pesan error, atau '' kalau berhasil dipicu.
  ...
  function buatBlokProduk(id, nama, barcode) {
    ...
    const IKON_NAMA = '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 7V4h16v3"/><path d="M9 20h6"/><path d="M12 4v16"/></svg>';
    const daftar = {
    ...
    function renderFormNama(t, d, w, field) {
      ...
      // .tm-input pakai flex (baru berlaku di dalam .tm-form); di panel ini bukan flex container,
      // jadi lebar dipaksa 100% supaya nama produk yang panjang tampil semua, tidak terpotong.
  ...
  if (diDalamFrame && new URL(location.href).searchParams.get(PARAM_IFRAME) === '1') {
    ...
    // Ctrl+S / Cmd+S di dalam iframe edit -> simpan (FAB-nya sendiri hanya ada satu, dirender di halaman induk)
    if (/\/edit/.test(location.pathname)) {
      document.addEventListener('keydown', e => {
        if ((e.ctrlKey || e.metaKey) && String(e.key).toLowerCase() === 's') {
          e.preventDefault();
          const err = simpanDiFrame(document, window);
          if (err && !/sedang memproses/.test(err)) alert(err);
        }
      });
    }
