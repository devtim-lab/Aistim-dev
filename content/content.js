(function() {
  ...
  // ===== DYNAMIC SCRIPTS (CSP-SAFE via userScripts API or skip) =====
  // Di site dengan CSP strict (seperti Erzap), script tag injection diblok.
  // Solusi: gunakan chrome.userScripts API (Chromium 120+) atau skip dengan pesan.
  function runDynamicScripts() {
    // Cek apakah chrome.userScripts API tersedia
    if (typeof chrome !== 'undefined' && chrome.userScripts && chrome.userScripts.register) {
      console.log('[Aistim] userScripts API tersedia — dynamic scripts dihandle oleh background');
      return;
    }
    // Fallback: script tag injection (akan diblok CSP di Erzap, tapi OK di site lain)
    ...
    chrome.storage.local.get(['effectiveScripts', 'scripts'], function(data) {
      var list = (data.effectiveScripts && data.effectiveScripts.length)
        ...
        : (data.scripts || []);
      var scripts = list.filter(function(s) { return s.enabled !== false && s.id !== 'erzap-001'; });
      ...
      scripts.forEach(function(s) {
        fetch(s.url, { cache: 'no-store' })
          ...
          .then(function(code) {
            // Inject via script tag — mungkin diblok CSP
            var tag = document.createElement('script');
            tag.textContent = code;
            tag.setAttribute('data-aistim-id', s.id);
            (document.head || document.documentElement).appendChild(tag);
            console.log('[Aistim] Dynamic script tag injected:', s.id);
          })
          .catch(function(err) {
            console.log('[Aistim] Dynamic skip:', s.id, err.message);
          });
      });
    });
  }
  ...
  function init() {
    ...
    runDynamicScripts(); // dynamic (mungkin diblok CSP di Erzap)
  }
